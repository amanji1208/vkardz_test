...
window.deployNFT = async function(name, symbol, network, primarySaleRecipient, platformFeeRecipient, image, platformFeeBasisPoints) {
  ...
  try {
    ...
    const contractAddress = await window.thirdwebSDK.deployer.deployBuiltInContract(
      "nft-collection",
      {
        name: name,
        symbol: symbol,
        primary_sale_recipient: primarySaleRecipient || window.ethers.constants.AddressZero,
        ...
        platform_fee_recipient: platformFeeRecipient || window.ethers.constants.AddressZero,
        platform_fee_basis_points: platformFeeBasisPoints // 1%
      },
...
window.deployERC20 = async function(name, symbol, initialSupply, decimals, network, primarySaleRecipient, platformFeeRecipient, image, platformFeeBasisPoints) {
  ...
  try {
    ...
    const contractAddress = await window.thirdwebSDK.deployer.deployBuiltInContract(
      "token",
      {
        ...
        decimals: decimals,
        image: image, // Replace with your token image URL
